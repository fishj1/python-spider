...
def write_excel_xls_append(path, value):
    ...
    for i in range(0, index):
        new_worksheet.write(i+rows_old, 1, value[i][0])  # 追加写入数据，注意是从i+rows_old行开始写入
        for j in range(0,5):              #前4列
            new_worksheet.write(i+rows_old, j, value[i][j])  # 统计数据从N开始 14
        for k in range(5, len(value[i])):             
            new_worksheet.write(i+rows_old, 8+k, value[i][k])  # 统计数据从N开始 14
...
if __name__ == '__main__':
    ...
    dir_path = "D:\python-work\python-spider"
    ...
    for path in all_files:
        ...
        result = ""
        for i in range(12,len(table0.rows)):  #读取第13行之后
            #读取第2列的数据           
        ...
        serial_num += 1
        dic_word.append(dic)
    xlsx_path = "D:\python-work\python-spider\案例分析-阶段-汇总test222.xlsx"
